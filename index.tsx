...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, size = "max-w-lg" }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
      <div className={`bg-white dark:bg-slate-800 rounded-[40px] shadow-2xl w-full ${size} overflow-hidden border border-white/10`}>
              ...
              <div className="flex justify-between items-start mb-16">
                 <div>
                    ...
                    <div className="flex gap-6 text-sm font-black text-slate-400 uppercase">
                       <span className="flex items-center gap-2"><Phone size={18} className="text-blue-500"/> {selected.phone || 'Girilmemiş'}</span>
                       <span className="flex items-center gap-2"><MapPin size={18} className="text-rose-500"/> {selected.city || 'Girilmemiş'}</span>
                    </div>
                 </div>
      ...
      <Modal isOpen={deleteConfirmModal} onClose={() => setDeleteConfirmModal(false)} title="Kaydı Kalıcı Olarak Sil">
         <div className="text-center space-y-6">
            ...
            <div className="flex gap-4">
               ...
               <button onClick={confirmDelete} className="flex-1 py-4 bg-rose-600 text-white rounded-[20px] font-black uppercase text-xs">Evet, Sil</button>
            </div>
         </div>
      </Modal>
...
const InvoicesView = ({ invoices, setInvoices, products, setProducts, setTransactions, settings }: any) => {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="relative">
        <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
        <input className="w-full pl-16 pr-6 py-5 bg-white dark:bg-slate-800 border rounded-[25px] outline-none font-bold" placeholder="Müşteri ismine göre faturayı bul..." value={search} onChange={e=>setSearch(e.target.value)} />
      </div>
      <div className="bg-white dark:bg-slate-800 rounded-[50px] shadow-sm border overflow-hidden">
        <table className="w-full text-left">
...
const TransactionsView = ({ transactions, setTransactions }: any) => {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex bg-slate-200 dark:bg-slate-800 p-1 rounded-[25px] w-fit">
